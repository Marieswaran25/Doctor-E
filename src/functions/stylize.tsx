...
export function stylize(str: string, navCopy?: string[], target?: string): React.ReactNode | React.ReactNode[] {
    ...
    if (italics_match) {
        ...
        return [before, <strong key={str}>{bold_match[0].replace('{bold}', '').replace('{/bold}', '')}</strong>, stylize(rest.join(bold_match[0]), navCopy, target)];
        ...
        return [
            before,
            <Link key={str} href={hyper_match[0].replace('{a}', '').replace('{/a}', '')} style={{ textDecoration: 'underline', color: 'blue' }} target="_blank" rel="noopener noreferrer">
        ...
        return [
            before,
            <Link key={str} href={nav} className="standard-link" target={target} rel="noopener noreferrer" style={{ textDecoration: 'underline', color: 'blue' }}>
