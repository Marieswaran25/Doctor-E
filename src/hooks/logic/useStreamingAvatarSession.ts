...
import StreamingAvatar, { ConnectionQuality, StartAvatarRequest, StreamingEvents, TaskMode, TaskType } from '@heygen/streaming-avatar';
...
export const useStreamingAvatarSession = () => {
    ...
    }, [handleStream, setSessionState, setStream, avatarRef, setIsListening, stopVoiceChat, clearMessages, setIsUserTalking, setIsAvatarTalking]);
    ...
    const start = useCallback(
        async (config: StartAvatarRequest, token?: string) => {
            ...
            avatarRef.current.on(StreamingEvents.CONNECTION_QUALITY_CHANGED, ({ detail }: { detail: ConnectionQuality }) => setConnectionQuality(detail));
